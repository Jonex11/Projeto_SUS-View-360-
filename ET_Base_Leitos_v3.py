...
# Driver Oracle é opcional: só é necessário quando --carregar-oracle é usado.
try:
    import oracledb
except ImportError:
    oracledb = None
...
CONFIG = {
    ...
    # DSN do Oracle Autonomous Database (TCPS). A senha NUNCA fica aqui:
    # use a variável de ambiente ORACLE_PASSWORD, --oracle-senha ou o
    # prompt interativo exibido em tempo de execução.
...
def gerar_url(ano):
    ...
    Padrão utilizado pelo Portal: Leitos_csv_YYYY.zip
    """
...
def baixar_arquivo(url, ano, forcar=False):
    ...
    for tentativa in range(1, CONFIG["TENTATIVAS_DOWNLOAD"] + 1):
        try:
            ...
            with requests.get(
                url, stream=True, timeout=CONFIG["TIMEOUT"]
            ) as response:
...
# COLUNAS A REMOVER
# =====================================================================
# Nomes já no padrão gerado por normalizar_coluna() (maiúsculas, sem
# caracteres especiais), pois o filtro é aplicado após a normalização.
COLUNAS_REMOVER = [
    "COMP",
    "CO_IBGE",
...
def filtrar_colunas(cabecalho):
    """
    Recebe o cabeçalho já normalizado (incluindo ANO_REFERENCIA) e retorna:
      - o cabeçalho sem as colunas listadas em COLUNAS_REMOVER;
      - a lista de índices originais que devem ser mantidos, usada para
        filtrar cada linha de dados na mesma posição.
    """
    indices_manter = [
        indice
...
def consolidar_csvs(arquivos, ano):
    ...
    incluindo a coluna ANO_REFERENCIA e salvando em UTF-8 com BOM.
    """
    ...
    with open(destino, "w", encoding="utf-8-sig", newline="") as saida:
        ...
        for arquivo in arquivos:
            ...
            with open(arquivo, "r", encoding=encoding, errors="replace", newline="") as entrada:
                ...
                # Primeiro CSV define o cabeçalho oficial
                if primeira_linha:
                    cabecalho_referencia = cabecalho
                    cabecalho_saida = cabecalho + ["ANO_REFERENCIA"]

                    # Remove as colunas indesejadas (ex.: dados de endereço,
                    # contato e demais colunas listadas em COLUNAS_REMOVER)
                ...
                for linha in leitor:
                    ...
                    # Aplica o mesmo filtro de colunas usado no cabeçalho
...
# ORACLE - CONECTAR
# =====================================================================
def conectar_oracle(dsn, usuario, senha):
    """
    Abre uma conexão com o banco Oracle usando o driver 'oracledb'
    (modo thin, não exige instalação do Oracle Instant Client).
    dsn no formato Easy Connect: host:porta/service_name
    """
    if oracledb is None:
        raise Exception(
            "Biblioteca 'oracledb' não instalada. Instale com: pip install oracledb"
        )
    logging.info(f"Conectando ao Oracle: usuário={usuario} | dsn={dsn}")
...
def criar_tabela_oracle_se_nao_existir(conexao, tabela, colunas):
    """
    Cria a tabela com colunas VARCHAR2(4000) genéricas caso ela ainda não
    exista no schema do usuário conectado. Uso opcional (--oracle-criar-tabela):
    se a tabela precisar de tipos específicos (NUMBER, DATE etc.), crie-a
    manualmente antes de rodar o script.
    """
    ...
    try:
        ...
        if existe:
            logging.info(f"Tabela {tabela} já existe. Estrutura mantida como está.")
            return
...
def carregar_dados_oracle(arquivo_csv, conexao, tabela, tamanho_lote, criar_tabela=False):
    """
    Lê o CSV consolidado (já com as colunas indesejadas removidas) e insere
    os registros na tabela Oracle informada, em lotes (executemany).
    Pressupõe que os nomes das colunas do CSV correspondem aos nomes das
    ...
    with open(arquivo_csv, "r", encoding="utf-8-sig", newline="") as entrada:
        ...
        logging.info(f"Colunas: {colunas_sql}")
        ...
        try:
            for linha in leitor:
                # Oracle trata string vazia como NULL para VARCHAR2; não é
                # necessária nenhuma conversão especial aqui.
...
def processar_ano(
    ...
    # 7. Carga no Oracle (opcional, habilitada via --carregar-oracle)
    total_oracle = None
    ...
    logging.info(f"Registros: {total:,}")
...
def obter_argumentos():
    ...
    parser = argparse.ArgumentParser(
        description="Download, extração e consolidação da base de Hospitais e Leitos SUS (CNES)."
    )
    parser.add_argument(
        "--ano",
        type=int,
    ...
    parser.add_argument(
        ...
        help="DSN Oracle (Easy Connect ou descritor completo). "
        "Padrão: Autonomous Database configurado no script.",
    )
    parser.add_argument(
        "--oracle-usuario",
        type=str,
    ...
    parser.add_argument(
        "--oracle-senha",
        type=str,
        help="Senha do banco Oracle. Prefira a variável de ambiente "
        "ORACLE_PASSWORD ou o prompt interativo em vez desta opção, "
        "para evitar que a senha fique visível no histórico do shell.",
    )
    parser.add_argument(
        "--oracle-tabela",
        type=str,
    ...
    parser.add_argument(
        ...
        help=f"Quantidade de registros por lote no INSERT (padrão: {CONFIG['ORACLE_TAMANHO_LOTE']}).",
    )
    parser.add_argument(
        "--oracle-criar-tabela",
        action="store_true",
        help="Cria a tabela Oracle automaticamente (colunas VARCHAR2(4000) "
        "genéricas) caso ela ainda não exista.",
    )
...
def main():
    ...
    # Conexão Oracle (opcional) — aberta uma única vez e reaproveitada para
    # todos os anos processados nesta execução.
    conexao_oracle = None
    if args.carregar_oracle:
        if not args.oracle_dsn or not args.oracle_usuario:
            logging.error(
                "Para usar --carregar-oracle informe --oracle-dsn e --oracle-usuario."
            )
            sys.exit(1)

        senha = args.oracle_senha or os.environ.get("ORACLE_PASSWORD")
        if not senha:
            senha = getpass.getpass("Senha Oracle: ")

        try:
            conexao_oracle = conectar_oracle(args.oracle_dsn, args.oracle_usuario, senha)
        except Exception as erro:
